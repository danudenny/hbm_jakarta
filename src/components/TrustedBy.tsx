...
const TrustedBy = () => {
  ...
  // Use the correct namespace with hyphen to match what's in Supabase
  const { t, i18n } = useTranslation("section.trusted-by");
  ...
  useEffect(() => {
    const fetchTrustedByData = async () => {
      ...
      try {
        ...
        Object.keys(localStorage).forEach((key) => {
          if (key.startsWith("i18next_section.trusted-by")) {
    ...
    // Reload translations for this section with the correct namespace
    i18n.reloadResources(currentLanguage, ["section.trusted-by"]);
