...
import { Save, ArrowLeft, Plus, Trash, Grip, FileText, Plane, CreditCard, MapPin, FileCheck, Users, 
  HelpCircle, Briefcase, Globe, Clock, Shield, Award, Star, Heart, Phone, Mail, Calendar, Book, 
  Clipboard, Key, Lock, Home, Building, Truck, ShoppingBag } from 'lucide-react';
...
const availableIcons = [
  'FileText', 'Plane', 'CreditCard', 'MapPin', 'FileCheck', 'Users',
  'Briefcase', 'Globe', 'Clock', 'Shield', 'Award', 'Star',
  'Heart', 'Phone', 'Mail', 'Calendar', 'Book', 'Clipboard',
  'Key', 'Lock', 'Home', 'Building', 'Truck', 'ShoppingBag'
];
        ...
        <div>
          ...
          <div className="flex items-center space-x-2">
            <div className="bg-primary/10 p-3 rounded-full text-primary">
              {React.createElement(iconMap[service.icon as keyof typeof iconMap] || iconMap.HelpCircle, { size: 24 })}
            </div>
            <button
              type="button"
                  ...
                  return (
                    <button
                      ...
                      className={`p-2 rounded-md hover:bg-primary/10 ${
                        service.icon === iconName ? 'bg-primary/20 text-primary' : 'text-gray-700'
                      }`}
                    >
...
const ServicesSectionEditor = () => {
    ...
    try {
      ...
      setFormData({
        ...
        content: {
          ...
          note: data.content.note || 'Need a service not listed above? We offer customized solutions for special cases.',
  ...
  const handleDragEnd = (event: any) => {
    ...
    if (active.id !== over.id) {
      setFormData((prev) => {
        const oldIndex = prev.content.services.findIndex(service => service.id === active.id);
        const newIndex = prev.content.services.findIndex(service => service.id === over.id);
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Edit Services Section</h1>
        </div>
        <button
        ...
        <>
            ...
            <div className="space-y-6">
              <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6">
                ...
                <p className="text-sm text-gray-500 mb-4">
                  Drag and drop to reorder services. Each service includes an icon, title, and description.
                </p>
                ...
                <div className="space-y-4">
                  ...
                  >
                    <SortableContext
                      items={formData.content.services.map(service => service.id)}
                      strategy={verticalListSortingStrategy}
                    >
                      {formData.content.services.map((service, index) => (
                        <SortableServiceItem
                          key={service.id}
                          service={service}
                          index={index}
                          ...
                          disableRemove={formData.content.services.length <= 1}
                        />
                      ))}
                    </SortableContext>
                  </DndContext>
              ...
              <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    />
                    <p className="mt-1 text-xs text-gray-500">
                      A brief introduction to your services that appears below the title
                    </p>
                  </div>
              ...
              <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    />
                    <p className="mt-1 text-xs text-gray-500">
                      This text appears above the CTA button
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6">
                ...
                <ul className="space-y-2 text-sm text-gray-600">
                  <li className="flex items-start">
                    ...
                    <p className="ml-2">Keep service descriptions concise and focused on benefits</p>
                  </li>
                  <li className="flex items-start">
                    <div className="flex-shrink-0 h-5 w-5 text-primary">
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <p className="ml-2">Use icons that visually represent each service</p>
                  </li>
                  <li className="flex items-start">
                    <div className="flex-shrink-0 h-5 w-5 text-primary">
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <p className="ml-2">Aim for 4-8 services for optimal display on most screens</p>
                  </li>
                  <li className="flex items-start">
                    <div className="flex-shrink-0 h-5 w-5 text-primary">
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <p className="ml-2">Use a clear call-to-action to guide visitors to the next step</p>
                  </li>
                </ul>
              </div>
            </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6">
            ...
            <div className="p-6 bg-gray-50 rounded-md">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 max-w-5xl mx-auto mb-8">
                {formData.content.services.map((service, index) => (
                  <div key={index} className="bg-white p-5 rounded-md border border-gray-200 shadow-sm">
                    <div className="flex items-center mb-3">
                      <div className="bg-primary/10 p-2 rounded-full text-primary mr-3">
                        {React.createElement(iconMap[service.icon as keyof typeof iconMap] || iconMap.HelpCircle, { size: 20 })}
                      </div>
                      <h4 className="font-medium">{service.title || "Service Title"}</h4>
                    </div>
              ...
              {formData.content.note && (
                <p className="text-center text-sm text-gray-500 italic mb-4 max-w-2xl mx-auto">
                  {formData.content.note}
                </p>
              )}
